...
export default function WatchPage() {
  ...
  useEffect(() => {
    ...
    setLikes(Number(localStorage.getItem(`likes-${video.slug}`) || 0));
    setLiked(localStorage.getItem(`liked-${video.slug}`) === "true");

    setSubscribers(Number(localStorage.getItem("raysstream-subscribers") || 0));
    setSubscribed(localStorage.getItem("raysstream-subscribed") === "true");

    const savedComments = localStorage.getItem(`comments-${video.slug}`);
