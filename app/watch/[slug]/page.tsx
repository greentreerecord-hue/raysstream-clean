...
const videos: Record<string, { title: string; file: string }> = {
  ...
  "its-cool": { title: "It's Cool", file: "/videos/itscool.mp4" },
  video1: { title: "It's Cool", file: "/videos/itscool.mp4" },

  video2: { title: "Video 2", file: "/videos/video2.mp4" },

  video3: { title: "Spaceship", file: "/videos/video3.mp4" },
  spaceship: { title: "Spaceship", file: "/videos/video3.mp4" },
};
...
export default function WatchPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    if (!video) return;
    const key = `raysstream_views_${slug}`;
  ...
  if (!video) {
    return (
      <main style={{ minHeight: "100vh", background: "#111", color: "white", padding: "30px", fontFamily: "Arial" }}>
        ...
        <p>Try /watch/itscool, /watch/video2, or /watch/video3</p>
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#111", color: "white", padding: "20px", fontFamily: "Arial" }}>
      <Link href="/" style={{ color: "red", fontSize: "18px" }}>← Back</Link>
      ...
      <video
        ...
        autoPlay
        playsInline
        style={{ width: "100%", maxWidth: "900px", background: "black", borderRadius: "12px" }}
      />

      <p style={{ fontSize: "22px" }}>Views: {views}</p>
    </main>
  );
} 
} 
